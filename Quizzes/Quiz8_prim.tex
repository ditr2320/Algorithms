...
\tikzstyle{green}=[circle ,top color =white , bottom color = processgreen!20 ,draw, processgreen , text=blue , minimum width =1 cm];
	...
	\path (D) edge node[above] {$6$} (F);
	\draw (E) edge node[below] {$3$} (F);
	\end{tikzpicture}  
\end{center}
\end{required}
...
\begin{proof}
Starting with vertex F and continuously adding the lightest edge: \\
Add edge FE \\
Add edge BE \\
Add edge BA \\
Add edge DE \\
Add edge CE \\
Final Order: FE,BE,BA,DE,CE
\end{proof}
